...
ifeq ($(CURDIR),$(DIR_BUILD))
	SONAME_$(1) := lib$(1)$(LIB_INFIX).so.$$(VERSION).$$(SUBVERSION)
	TARGET_$(1) := lib$(1)$(LIB_INFIX).so.$$(RELEASE)
	TARGETS += $$(TARGET_$(1))
...
define CONFIGURE_DRIVER
	SONAME_$(1) := lib$(1)_driver$(LIB_INFIX).so.$$(VERSION).$$(SUBVERSION)
	TARGET_$(1)_driver := lib$(1)_driver$(LIB_INFIX).so.$$(RELEASE)
