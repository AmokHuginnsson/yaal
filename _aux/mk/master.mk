...
build/%/$(1): configure $(1).in
	@$$(eval DIR = $$(dir $$(@))) \
		...
		if test -t 1 -a "x$${VERBOSE}" != "xyes" ; then \
			/bin/rm -f $$(notdir $$(@)) && \
			../../configure $$(CONF_$$(*)) $$(if $$(PREFIX),--prefix=$$(PREFIX)) $$(if $$(SYSCONFDIR),--sysconfdir=$$(SYSCONFDIR)) $$(CONFIGURE) | tee -a make.log | awk -v CL="`tput cr;tput dl1`" '{printf CL"%s\r", $$$$0}' ; \
		else \
			../../configure $$(CONF_$$(*)) $$(if $$(PREFIX),--prefix=$$(PREFIX)) $$(if $$(SYSCONFDIR),--sysconfdir=$$(SYSCONFDIR)) $$(CONFIGURE) | tee -a make.log ; \
