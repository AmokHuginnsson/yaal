...
public:
	 ...
	 *
	 * \tparam T - type of compile time constants to insert into this deque.
	 * \param constants_ - set of compile time constants to into into this deque.
	 */
	template<typename T>
	HDeque( std::initializer_list<T> constants_ )
		: _chunks(), _start( 0 ), _size( 0 ) {
		M_PROLOG
...
typename HDeque<type_t, allocator_t>::iterator HDeque<type_t, allocator_t>::erase( iterator first_, iterator last_ ) {
	...
	if ( toRemove ) {
		...
		if ( first_._index < ( _size - last_._index ) ) /* Move front. */ {
			...
			for (
				int long chunkIndex( _start / VALUES_PER_CHUNK ), newFirstChunkIndex( ( _start + toRemove ) / VALUES_PER_CHUNK );
			...
			int long trimAt( ( _start + _size - toRemove ) - 1 );
			trimAt = ( ( trimAt >= _start ) ? trimAt : ( _start + _size - toRemove ) );
			for (
				int long chunkIndex( ( trimAt / VALUES_PER_CHUNK ) + ( ( _size - toRemove ) > 0 ? 1 : 0 ) );
