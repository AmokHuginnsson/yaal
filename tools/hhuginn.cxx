...
HRule huginn_grammar( void ) {
	...
	HRule ifStatement( "ifStatement", executing_parser::constant( "if" ) >> '(' >> booleanExpression >> ')' >> '{' >> statementList >> '}' >> -( constant( "else" ) >> '{' >> statementList >> '}' ) );
	HRule whileStatement( "whileStatement", constant( "while" ) >> '(' >> booleanExpression >> ')' >> '{' >> statementList >> '}' );
