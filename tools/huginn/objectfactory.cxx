...
HObjectFactory::HObjectFactory( HRuntime* runtime_ )
	...
	, _stringPool( _string.raw() )
	, _integerPool( _integer.raw() )
	, _booleanPool( _boolean.raw() )
	, _realPool( _real.raw() )
	, _numberPool( _number.raw() )
	, _characterPool( _character.raw() )
	, _listPool( _list.raw() )
	, _dequePool( _deque.raw() )
	, _dictPool( _dict.raw() )
	, _lookupPool( _lookup.raw() )
	, _orderPool( _order.raw() )
	, _setPool( _set.raw() )
	, _referencePool()
