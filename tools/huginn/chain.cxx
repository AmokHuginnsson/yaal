...
HHuginn::class_t HChain::get_class( HRuntime* runtime_, huginn::HClass const* origin_ ) {
	M_PROLOG
	HHuginn::class_t c(
...
protected:
	...
	int _colIdx;
	huginn::HIterable::iterator_t _it;
public:
	HChainIterator( HThread* thread_, HHuginn::values_t& source_, int position_ )
		...
		if ( _colIdx < _source.get_size() ) {
			_it = static_cast<huginn::HIterable*>( _source.front().raw() )->iterator( thread_, position_ );
...
private:
	void ensure( HThread* thread_, int position_ ) {
		while ( ! _it->is_valid( thread_, position_ ) ) {
			...
			_it = static_cast<huginn::HIterable*>( _source[_colIdx].raw() )->iterator( thread_, position_ );
		}
		return;
	}
};
