...
int HDataProcess::init_xrc ( char const * a_pcProcessName,
	...
	f_psRootMenu = build_menu ( l_sNode, a_roHandlers );
...
OMenuItem * HDataProcess::build_menu ( HXml::ONode & a_rsNode,
	...
	for ( l_psNode = & a_rsNode.f_oChilds.go ( 0 ); l_psNode;
		...
		if ( l_psNode->f_oName == "label" )
			l_sMenuItem.f_oLabel = l_psNode->f_oContents.head ( );
		...
		else if ( ( l_psNode->f_oName == "menu" ) || ( l_psNode->f_oName == "menu_item" ) )
			l_sMenuItem.f_psSubMenu = build_menu ( * l_psNode, a_roHandlers );
		else
