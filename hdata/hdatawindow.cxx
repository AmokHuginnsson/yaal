	...
	switch ( a_eMode )
		...
		case ( DOCUMENT::D_EDIT ):
			...
			for ( controls_t::iterator it = f_oEditModeControls.begin(); it != end; ++ it )
				(*it)->enable( a_eMode == DOCUMENT::D_EDIT );
			end = f_oViewModeControls.end();
			for ( controls_t::iterator it = f_oViewModeControls.begin(); it != end; ++ it )
				(*it)->enable( a_eMode == DOCUMENT::D_VIEW );
			controls_t::iterator begin = a_eMode == DOCUMENT::D_EDIT ? f_oEditModeControls.begin() : f_oViewModeControls.begin();
			end = a_eMode == DOCUMENT::D_EDIT ? f_oEditModeControls.end() : f_oViewModeControls.end();
			if ( begin != end )
				(*begin)->set_focus();
			}
		break;
...
int HDataWindow::handler_edit( int, void const* )
	...
	f_eMode = HSQLDescriptor::MODE::D_UPDATE;
	set_mode( DOCUMENT::D_EDIT );
	return ( 0 );
	M_EPILOG
	}
...
int HDataWindow::handler_delete( int, void const* )
	...
	if ( f_poMainControl )
		...
		f_oDB->execute( HSQLDescriptor::MODE::D_DELETE );
		}
	f_poMainControl->load();
	return ( 0 );
	M_EPILOG
...
int HDataWindow::handler_save( int, void const* )
	{
	M_PROLOG
	if ( f_eDocumentMode != DOCUMENT::D_EDIT )
		{
		f_oStatusBar->message( COLORS::D_FG_BRIGHTRED, _( "There is nothing to save." ) );
		return ( 0 );
		}
	if ( f_eMode == HSQLDescriptor::MODE::D_UPDATE )
		{
		HString filter;
		filter.format( "id = %ld", f_poMainControl->get_current_id() );
		f_oDB->set_filter( filter );
...
int HDataWindow::handler_cancel( int, void const* )
	...
	if ( f_eDocumentMode != DOCUMENT::D_EDIT )
		return ( 0 );
	set_mode( DOCUMENT::D_VIEW );
	if ( ( f_eMode == HSQLDescriptor::MODE::D_INSERT ) && f_poMainControl )
